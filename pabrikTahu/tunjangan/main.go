...
import (
	"fmt"
	//local module
	ep "git.bluebird.id/pabrikTahu/tunjangan/endpoint"
...
func main() {
	...
	var server pb.TunjanganServiceServer
	{
		...
		//buat service sebelum melakukan code di bawah
		...
		service := svc.NewTunjangan(dbReadWriter)
		endpoint := ep.NewTunjanganEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCTunjanganServer(endpoint, tracer, logger)
	}
	...
	pb.RegisterTunjanganServiceServer(grpcServer, server)
